...
export function BodyShopServices() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Animate Body Shop Section
      const bsText = bodyShopSectionRef.current?.querySelector(".bs-text");
      const bsImage = bodyShopSectionRef.current?.querySelector(".bs-image");
  ...
  // Get Body Shop Data (Index 2 from SERVICE_DETAILS based on description "Body Shop & Painting" being 3rd usually, but let's check SERVICE_DETAILS in data.ts)
  // Checking data.ts from previous context: SERVICE_DETAILS[2] is "BODY SHOP"
  const bodyShopData = SERVICE_DETAILS[2];
  ...
  return (
    <div ref={containerRef} className="flex flex-col w-full" dir={direction}>
      ...
      {/* STEP 2: BODY SHOP SECTION */}
      <div
        ref={bodyShopSectionRef}
        ...
        <div className="container mx-auto px-6">
          <div className="text-center mb-16 space-y-4">
            <h2 className="font-heading text-3xl md:text-5xl font-bold tracking-tight text-secondary uppercase">
              {t("bodyShopSection.title")}
            </h2>
            <p className="font-sans text-muted-foreground max-w-2xl mx-auto">
              {t("bodyShopSection.description")}
            </p>
          </div>
